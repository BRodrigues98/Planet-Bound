...
public class GamePlay implements Serializable {
    ...
    public String getMiningResults() {
        if(data.getShip().getDrone().getResourceFound() != null)
            ...
            else if(data.getShip().getNumArtifacts() != 5)
                return "Your drone found an artifact! You need " + (5 - data.getShip().getNumArtifacts()) + " to win the game!";
            else
                return "You've found all the artifacts! You've won the game";
        else
            return null;
    }
    ...
    public int getCargoHoldLvl() {
        return data.getShip().getCargoHoldLvl();
    }

    public boolean hasWon() {
        return data.getShip().getNumArtifacts() == 5;
    }
    //------------------------------------
}
