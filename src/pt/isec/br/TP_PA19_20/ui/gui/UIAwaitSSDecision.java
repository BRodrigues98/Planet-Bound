            ...
            public void handle(ActionEvent actionEvent) {
                ...
                if(!dgObs.isCargoMaxLevel()) {
                    if (dgObs.getBlackRes() <= 1 || dgObs.getRedRes() <= 1 || dgObs.getBlueRes() <= 1 || dgObs.getGreenRes() <= 1) {
                        ...
                        alert.setTitle("Cargo Hold upgraded");
                        alert.setContentText("Cargo Hold upgraded to level " + dgObs.getCargoHoldLvl() + ". You can now hold " + (dgObs.getCargoHoldLvl() * 6) + " of each resource.");
